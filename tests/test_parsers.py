...
class TestParsers(TestBase):
    ...
    def testFilter(self):
        ...
        var_reads_P={'UW_Freq': '0.0005','EXAC': '-1',  'Var_Reads': '8','1000g_ALL': '-1', 'Variant_Type': 'frameshift deletion,exonic'}
        ...
        self.assertTrue(parsers.opx_bro_filter(vtype, variant_to_include, uw_freq_P))
        self.assertFalse(parsers.opx_bro_filter(vtype, variant_to_include, uw_freq_F))
        self.assertTrue(parsers.opx_bro_filter(vtype, variant_to_include, exac_P))
        self.assertFalse(parsers.opx_bro_filter(vtype, variant_to_include, exac_F))
        self.assertTrue(parsers.opx_bro_filter(vtype, variant_to_include, var_reads_P))
        self.assertFalse(parsers.opx_bro_filter(vtype, variant_to_include, var_reads_F))
        self.assertTrue(parsers.opx_bro_filter(vtype, variant_to_include, thouG_P))
        self.assertFalse(parsers.opx_bro_filter(vtype, variant_to_include, thouG_F))
        self.assertTrue(parsers.opx_bro_filter(vtype, variant_to_include, var_type_P))
        self.assertFalse(parsers.opx_bro_filter('intronic', variant_to_include, var_type_F))
